...
//            break;
    ...
    {
        $user = $this->_userdata->idUser();
        if ($this->is_ajax()) {
            if ($this->is_post()) {
                foreach ($this->post('tarifas') as $key => $row) {
                    $row = (object)$row;
                    $id = ORM::$_db->query("REPLACE INTO cu_tarifas(empresa,tarifa,precio) VALUES ('{$user}','{$row->id}','{$row->precio}')");
                }
            }
        }
        die(json_encode(array()));
